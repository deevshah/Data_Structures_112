...
public class RUHungry {
    ...
    public void menu(String inputFile) {
        ...
        MenuNode first=new MenuNode(null, null);

        for (int i = 0; i < length; i++) {
            String category = StdIn.readString();
            categoryVar[i] = category;

            int dishLength = StdIn.readInt();
            first = null;
            for (int j = 0; j < dishLength; j++) {
                String dishName = StdIn.readString() + StdIn.readLine();
                int ingredientLength = StdIn.readInt();
                int[] ingredeints = new int[ingredientLength];

                for (int k = 0; k < ingredientLength; k++) {
                    ingredeints[k] = StdIn.readInt();
                }

                Dish dish = new Dish(category, dishName, ingredeints);

                MenuNode menu = new MenuNode(dish, null);
                menu.setNextMenuNode(first);
                first = menu;
            }
            menuVar[i] = first;
    ...
    public void addStockNode(StockNode newNode) {
        ...
        {
            newNode.setNextStockNode(stockVar[hash]);
            stockVar[hash] = newNode;
        }
    ...
    public StockNode findStockNode(int ingredientID) {
        ...
        StockNode node = stockVar[hash];
        StockNode stock = null;
        ...
        return stock; // update the return value
    }
    ...
    public void updateStock(String ingredientName, int ingredientID, int stockAmountToAdd) {
        ...
        int newStock = node.getIngredient().getStockLevel() + stockAmountToAdd;
        ingredient.setStockLevel(newStock);
        node.setIngredient(ingredient);
    ...
    public void updatePriceAndProfit() {
        ...
        for (int i=0; i<menuVar.length;i++)
        {
            ...
            while (ptr!=null)
            {
                ...
                int[] stock = dish.getStockID();
                double cost = 0;

                for (int j=0;j<stock.length;j++)
                {
                    cost += findStockNode(stock[j]).getIngredient().getCost();
                }
    ...
    public void createStockHashTable(String inputFile) {
        ...
        while (!StdIn.isEmpty())
        {
            ...
            Ingredient ingredient = new Ingredient(id, item, stock, price);
    ...
    public boolean checkDishAvailability(String dishName, int numberOfDishes) {
        ...
        for (int i =0; i<stockID.length;i++)
        {
            ...
            int stocklevel = stocknode.getIngredient().getStockLevel();
    ...
    public void order(String dishName, int quantity) {
        ...
        // WRITE YOUR CODE HERE
        boolean b = checkDishAvailability(dishName, quantity);

        if (b){
            double profit = (findDish(dishName).getDish().getProfit())*quantity;
            TransactionData order1 = new TransactionData("order", dishName, quantity, profit, true);
            ...
            for (int i=0; i<stockID.length;i++)
            {
                updateStock(null, stockID[i], -quantity);
            }
        ...
        if (!b)
        {
            TransactionData order2 = new TransactionData("order", dishName, quantity, 0.0, false);
            ...
            while(ptr!=null)
            {
                ...
                {
                    double profit1 = (ptr.getDish().getProfit())*quantity;
                    TransactionData order3 = new TransactionData("order", ptr.getDish().getName(), quantity, profit1, true);
                    addTransactionNode(order3);
        
                    int[] stockID2 = ptr.getDish().getStockID();

                for (int i=0; i<stockID2.length;i++)
                {
                    updateStock(null, stockID2[i], -quantity);
                }
                    return;
                }
                else{
                    TransactionData order9 = new TransactionData("order", ptr.getDish().getName(), quantity, 0.0, false);
                    addTransactionNode(order9);
                }
                ptr = ptr.getNextMenuNode();
            }
            ...
            int index=findCategoryIndex(node.getDish().getCategory());

            MenuNode first = menuVar[index];
            MenuNode ptr2 = first;
            
                while(!(ptr2.getDish().getName().equals(dishName))  && (ptr2!=null))
                {
                    if(checkDishAvailability(ptr2.getDish().getName(), quantity))
                {
                    double profit1 = (ptr2.getDish().getProfit())*quantity;
                    TransactionData order4 = new TransactionData("order", ptr2.getDish().getName(), quantity, profit1, true);
                    addTransactionNode(order4);
                    Dish dish = ptr2.getDish();
                    int[] stockID3 = dish.getStockID();

            for (int i=0; i<stockID3.length;i++)
            {
                updateStock(null, stockID3[i], -quantity);
            }
                    return;
                }
                else{
                    TransactionData order10 = new TransactionData("order", ptr2.getDish().getName(), quantity, 0.0, false);
                    addTransactionNode(order10);
                }
                ptr2 = ptr2.getNextMenuNode();
                }
            }
        }
    ...
    public double profit() {
        ...
        while(ptr!=null){
            if (ptr.getData().getSuccess()==true)
            {
    ...
    public void donation(String ingredientName, int quantity) {
        ...
        int stockLevel = node.getIngredient().getStockLevel();
    ...
    public void restock(String ingredientName, int quantity) {
        ...
        if (profit>=cost){
            ...
            TransactionData data2 = new TransactionData("restock", ingredientName, quantity, 0, false);
            addTransactionNode(data2);
        }
    }
